...
| activate | The only skill with external effect — sends an approved payload from `outputs/` to its configured destination, with a human gate and a receipt. No destination is configured today (Jira and Docmost were dropped). |
| new-project | The skill that onboards a full project: folder from template, routing entry. |
...
| ProDesk | The physical/Tailscale server Keeis operates: hosts the bare Git origin and the cadence cron. |
