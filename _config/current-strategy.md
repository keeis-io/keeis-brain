...
2. Business plan reviewable by Erik → `business-plan/` pipeline. Not tied to a specific project:
   ...
   the concrete example on hand then, not a requirement that the plan stay scoped to `plates`.

## Open question #1
**Who pays, for what concrete pain, and how much?**
Until this is answered with evidence, no new features get written and no technical scope gets added.

## Current hypotheses
- **Primary — sole active discovery focus**: consultative-sales-teams, high-ticket B2B sales
...
- **Paused, not disproven**: informal microenterprise tax/CCSS/municipal compliance — was logged
  2026-08-16 as a parallel exception to the 2-front limit; paused as of 2026-08-26, same reason as
  `plates` above (all focus now on consultative-sales-teams) —
  `discovery/01_hypotheses/output/2026-08-16-hypothesis-formalizacion-microempresas.md`.

## Discarded for now
(empty — `plates` is paused, not disproven, so it stays out of this section; whatever actually
gets disproven gets logged here with date and reason)
